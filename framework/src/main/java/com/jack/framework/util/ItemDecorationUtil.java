...
public class ItemDecorationUtil {
    ...
    public static void topAuto(Rect outRect, View view, RecyclerView parent, int
        ...
        if (childPosition > 0) {
            outRect.top = DensityUtil.dip2px(FrameWorkApplication.getApplication(), space);
        }
    }
    ...
    public static void setLeftAndBottomAuto(Rect outRect, View view, RecyclerView parent, int
            space) {
        outRect.top = DensityUtil.dip2px(FrameWorkApplication.getApplication(), space);
        outRect.right = DensityUtil.dip2px(FrameWorkApplication.getApplication(), space);
        /**
         * 计算底部
         * */
        int childPosition = parent.getChildAdapterPosition(view);
        ...
        if (flag == 0) {
            //只有一行的话
            outRect.bottom = DensityUtil.dip2px(FrameWorkApplication.getApplication(), space);
            ...
            if ((childPosition + 1) % gridLayoutManager.getSpanCount() == 1) {
                outRect.left = DensityUtil.dip2px(FrameWorkApplication.getApplication(), space);
            }
